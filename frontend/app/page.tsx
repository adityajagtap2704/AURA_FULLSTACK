      ...
      <div className="max-w-[1000px] mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {INTEGRATIONS.map((int, i) => (
            <motion.div key={int.name} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.08}}
              whileHover={{y:-6, boxShadow:'0 20px 50px rgba(31,27,22,0.10)'}}
              className="flex flex-col items-center text-center p-8 bg-white rounded-[20px] border border-[#EDE8E0] cursor-pointer transition-all duration-300 shadow-sm">
              <div className="w-[72px] h-[72px] rounded-[18px] flex items-center justify-center mb-5 shadow-sm" style={{background:int.bg}}>
                {int.logo}
              </div>
              <h3 className="text-[15px] font-bold text-[#1F1B16] mb-1.5">{int.name}</h3>
              <p className="text-[12px] text-[#9B8F85] leading-snug max-w-[160px]">{int.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
const STEPS = [
  ...
  {icon:(
    <svg className="w-7 h-7 text-[#C17817]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10"/><path d="M20.49 15a9 9 0 01-14.85 3.36L1 14"/></svg>
  ),label:'We sync your data securely',desc:'Real-time sync across all connected tools.'},
  {icon:(
    <svg className="w-7 h-7 text-[#C17817]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z"/></svg>
  ),label:'AI organizes everything',desc:'Smart summaries and priority queues daily.'},
  {icon:(
    <svg className="w-7 h-7 text-[#C17817]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
  ),label:'You get clarity and get things done',desc:'Start every day with a clear action plan.'},
];
const STATS = [
  {icon:'⊞',value:'4+',label:'Connected Apps'},
  {icon:'🔒',value:'100%',label:'Secure & Private'},
  {icon:'☰',value:'1',label:'Unified Dashboard'},
  {icon:'⟳',value:'24/7',label:'Smart Sync'},
];

function HowItWorks() {
  return (
    <section id="how-it-works" className="py-24 bg-[#FDFAF6]">
      <div className="max-w-[1000px] mx-auto px-6">
        <motion.h2 initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div className="hidden md:block absolute top-[34px] left-[16%] right-[16%] h-0 border-t border-dashed border-[#DDD5C8]"/>
          {STEPS.map((s,i)=>(
            <motion.div key={s.label} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.1}}
              className="flex flex-col items-center text-center">
              <motion.div whileHover={{scale:1.08,rotate:4}}
                className="w-[68px] h-[68px] rounded-[18px] bg-[#FDF4E7] border border-[#EDD9A3] flex items-center justify-center mb-4 shadow-sm relative z-10 transition-all duration-300">
                {s.icon}
              </motion.div>
              <h3 className="text-[13px] font-bold text-[#1F1B16] mb-2 leading-snug">{s.label}</h3>
              <p className="text-[12px] text-[#9B8F85] leading-relaxed">{s.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mt-20">
          {STATS.map((s,i)=>(
            <motion.div key={s.label} initial={{opacity:0,scale:0.95}} whileInView={{opacity:1,scale:1}} viewport={{once:true}} transition={{delay:i*0.08}}
              whileHover={{y:-4}}
              className="bg-white rounded-[20px] p-6 border border-[#EDE8E0] shadow-sm hover:shadow-md flex items-center gap-4 transition-all duration-300">
              <div className="w-12 h-12 rounded-xl bg-[#FDF4E7] flex items-center justify-center text-2xl shrink-0">{s.icon}</div>
              <div>
                <div className="text-[24px] font-black text-[#1F1B16] leading-none">{s.value}</div>
          ...
          {TESTIMONIALS.map((t,i)=>(
            <motion.div key={t.name} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.1}}
              ...
              <div className="flex items-center gap-3 pt-3 border-t border-[#F0EBE3]">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FDF4E7] to-[#F0DDB4] border border-[#E8D5B0] flex items-center justify-center text-xl">{t.avatar}</div>
                <div>
