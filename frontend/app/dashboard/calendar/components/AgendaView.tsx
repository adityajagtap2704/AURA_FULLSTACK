      ...
      {sortedKeys.length > 0 ? sortedKeys.map(key => {
        ...
        return (
          <div key={key} className="rounded-2xl overflow-hidden border border-border bg-card shadow-sm">
            {/* Date header */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-muted/30 dark:bg-white/[0.025]">
              <div className="flex items-center gap-3">
                {/* Calendar date chip */}
                <div className="h-10 w-10 rounded-xl border border-primary/20 flex flex-col items-center justify-center shrink-0 bg-primary/8 dark:bg-primary/10">
                  <span className="text-[8px] font-black uppercase tracking-widest text-primary leading-none">{monthShort}</span>
                  <span className="text-[16px] font-black text-primary leading-none mt-0.5">{dayNum}</span>
                </div>
                <div>
                ...
                return (
                  <div key={event.id}
                    className="flex rounded-xl overflow-hidden border border-border
                               ...
                               hover:shadow-md dark:hover:shadow-black/40 transition-all duration-200 group">
                    {/* Colour stripe */}
                    ...
                    <div className="flex flex-1 items-center gap-3.5 px-4 py-3">
                      {/* Icon */}
                      <div className="h-9 w-9 rounded-lg shrink-0 flex items-center justify-center border border-border/50"
                           style={{ background: a.lightBg, boxShadow: `0 0 0 2px ${a.hex}18` }}>
                        {isGoogle ? <GoogleIcon size={15} /> : <span className="h-2.5 w-2.5 rounded-full" style={{ background: a.hex }} />}
                      </div>
