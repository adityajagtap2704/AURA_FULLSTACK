...
describe('connector isolation', () => {
  ...
  it('NotionConnector.mapToCanonical produces well-formed canonical Tasks/Documents with a valid source', async () => {
    ...
    const raw = {
      databaseItems: [
        {
          id: 'page_1',
          properties: {
            Name: { title: [{ plain_text: 'Ship AURA' }] },
      ...
      pages: [
        { id: 'page_2', url: 'https://notion.so/page_2', last_edited_time: '2026-07-14T00:00:00Z', properties: { Name: { title: [{ plain_text: 'Notes' }] } } },
      ],
    };
    ...
    expect(canonical.tasks[0]).toMatchObject({ source: 'notion', source_id: 'page_1', title: 'Ship AURA', status: 'In Progress' });
